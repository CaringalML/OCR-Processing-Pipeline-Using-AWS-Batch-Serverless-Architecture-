...
def lambda_handler(event, context):
    ...
    try:
        ...
        if search_term and not fuzzy:
            # Exact search in OCR results - search both raw OCR text and refined text
            ocr_filter = Attr('ocr_text').contains(search_term)
            refined_filter = Attr('refined_text').contains(search_term)
            combined_filter = ocr_filter | refined_filter
            ...
            if status in ['completed', 'processed']:
                try:
                    ...
                    if 'Item' in ocr_response:
                        ...
                        # Add snippet for search results - prioritize refined_text for better semantic content
                        if search_term:
                            # Use refined_text if available, fallback to raw ocr_text
                            search_text = ocr_item.get('refined_text', ocr_item.get('ocr_text', ''))
                            
                            if search_text:
                                if fuzzy:
                                    # Fuzzy search in refined text for better semantic matching
                                    snippet, score = fuzzy_search_in_text(search_term, search_text, fuzzy_threshold)
                                    if snippet:
                                        result_item['snippet'] = snippet
                                        result_item['fuzzyScore'] = score
                                        match_score = min(match_score, score)
                                        fuzzy_matched = True
                                    elif search_term:  # If search term provided but no fuzzy match
                                        continue
                                else:
                                    # Exact search in refined text
                                    index = search_text.lower().find(search_term.lower())
                                    if index != -1:
                                        # Extract snippet around search term
                                        start = max(0, index - 100)
                                        end = min(len(search_text), index + len(search_term) + 100)
