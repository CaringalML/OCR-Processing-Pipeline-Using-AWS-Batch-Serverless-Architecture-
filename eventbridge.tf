...
resource "aws_cloudwatch_event_rule" "batch_job_state_change_rule" {
  ...
  event_pattern = jsonencode({
    ...
    detail = {
      jobQueue  = [aws_batch_job_queue.main[0].arn]
...
resource "aws_cloudwatch_event_target" "lambda_batch_reconciliation" {
  count     = var.deployment_mode == "full" ? 1 : 0
  rule      = aws_cloudwatch_event_rule.batch_job_state_change_rule[0].name
  target_id = var.eventbridge_batch_reconciliation_target_id
  arn       = aws_lambda_function.batch_status_reconciliation[0].arn
}
...
resource "aws_lambda_permission" "allow_eventbridge_batch_reconciliation" {
  ...
  function_name = aws_lambda_function.batch_status_reconciliation[0].function_name
  principal     = var.eventbridge_lambda_principal
  source_arn    = aws_cloudwatch_event_rule.batch_job_state_change_rule[0].arn
}
...
resource "aws_cloudwatch_event_target" "lambda_dead_job_detector" {
  count     = var.deployment_mode == "full" ? 1 : 0
  rule      = aws_cloudwatch_event_rule.dead_job_detection_rule[0].name
  target_id = var.eventbridge_dead_job_detection_target_id
  arn       = aws_lambda_function.dead_job_detector[0].arn
}
...
resource "aws_lambda_permission" "allow_eventbridge_dead_job_detector" {
  ...
  action        = var.eventbridge_lambda_action
  function_name = aws_lambda_function.dead_job_detector[0].function_name
  principal     = var.eventbridge_lambda_principal
  source_arn    = aws_cloudwatch_event_rule.dead_job_detection_rule[0].arn
}